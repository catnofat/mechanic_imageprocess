...
for imagepath in imgfiles:
    ...
    #if len(dotlist) !=9:
    cv2.imwrite('cam(0)'+str(a)+'.png',binaryImage)
    #cv2.imshow('bmask', binaryImage)
    #cv2.imshow('mask', mask)
    #cv2.waitKey(1)
print("time :", time.time() - start)  # 현재시각 - 시작시간 = 실행 시간
df = pd.DataFrame(alldot)
np.save('List1.npy', npalldot)
df.to_csv("List_csv1.csv", header =None, index = None)
